...
import type { Status, StatusType } from '../../hooks/useStatus';
...
function Alert({ type, icon, className, children }: Props): React.JSX.Element {
...
Alert.WithStatus = function AlertWithStatus({
  status,
  ...props
}: AlertWithStatusProps): React.JSX.Element | null {
  if (!status) {
    return null;
  }
  return (
