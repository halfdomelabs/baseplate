...
var PrismaPasswordTransformerGenerator = (0, sync_1.createGeneratorWithChildren)({
    ...
    createGenerator: function (descriptor, _a) {
        var _this = this;
        var prismaCrudServiceSetup = _a.prismaCrudServiceSetup, passwordHasherService = _a.passwordHasherService;
        prismaCrudServiceSetup.addTransformer('password', {
            buildTransformer: function () { return ({
                inputFields: [
                    {
                        type: core_generators_1.TypescriptCodeUtils.createExpression('string | null'),
                        dtoField: {
                            name: 'password',
                            type: 'scalar',
                            scalarType: 'string',
                ...
                outputFields: [
                    {
                        name: 'passwordHash',
                        transformer: core_generators_1.TypescriptCodeUtils.createBlock('const passwordHash = password == null ? password : (await hasherService.hash(password));', 'import {hasherService} from "%password-hasher-service";', { importMappers: [passwordHasherService] })
                    },
                ],
                isAsync: true
            }); }
...
exports["default"] = PrismaPasswordTransformerGenerator;
