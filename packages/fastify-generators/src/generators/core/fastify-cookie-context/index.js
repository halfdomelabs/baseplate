...
var request_service_context_1 = require("../request-service-context");
var descriptorSchema = zod_1.z.object({
    placeholder: zod_1.z.string().optional()
});
var FastifyCookieContextGenerator = (0, sync_1.createGeneratorWithChildren)({
    ...
    createGenerator: function (descriptor, _a) {
        var _this = this;
        var node = _a.node, fastifyServer = _a.fastifyServer, requestServiceContextSetup = _a.requestServiceContextSetup;
        node.addPackages({
            '@fastify/cookie': '7.0.0'
        });
        fastifyServer.registerPlugin({
            name: 'cookies',
            plugin: new core_generators_1.TypescriptCodeExpression('fastifyCookie', "import fastifyCookie from '@fastify/cookie'")
        });
        requestServiceContextSetup.addContextField({
            name: 'cookieStore',
            type: core_generators_1.TypescriptCodeUtils.createExpression('CookieStore', undefined, {
                headerBlocks: [
                    core_generators_1.TypescriptCodeUtils.createBlock("\ninterface CookieStore {\n  get(name: string): string | undefined;\n  set(name: string, value: string, options?: CookieSerializeOptions): void;\n  clear(name: string): void;\n}\n", "import { CookieSerializeOptions } from '@fastify/cookie';"),
                ]
            }),
            body: function (req, reply) {
                return new core_generators_1.TypescriptCodeBlock("function getReply(): FastifyReply {\n          if (!".concat(reply, ") {\n            throw new Error(\n              'Reply is not defined. This may happen if calling this function from a websocket connection.'\n            );\n          }\n          return ").concat(reply, ";\n        }\n      "));
            },
            creator: function (req) {
                return new core_generators_1.TypescriptCodeExpression("\n{\n  get: (name) => ".concat(req, ".cookies[name],\n  set: (name, value, options) => getReply().setCookie(name, value, options),\n  clear: (name) => getReply().clearCookie(name),\n}\n"));
            }
        });
        return {
...
exports["default"] = FastifyCookieContextGenerator;
