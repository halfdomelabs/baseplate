...
import type { NotificationService } from '$servicesNotificationService';

import { isNotificationTopicKey } from '$constantsNotificationTopics';
import { createSigner } from '%appSecretImports';
import { getApiUrl } from '%appUrlsImports';
...
export async function applyUnsubscribeLink(
  ...
  const recipient = await TPL_USER_DELEGATE.findUnique({
