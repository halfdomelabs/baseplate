...
const STORAGE_FILE_CATEGORIES_DOC = `# File Storage Categories
      ...
      agentDocCompiler({
        ...
        compile: ({ projectDefinition }) => {
          ...
          if (storage.fileCategories.length > 0) {
            docs['storage-file-categories'] = {
              id: 'storage-file-categories',
              description:
                'how to configure and use file storage categories (allowed types, size limits, upload roles)',
              content: STORAGE_FILE_CATEGORIES_DOC,
            };
          }
