...
> {
  ...
  /** Eligible channels; only registered ones are delivered to. */
  channels: string[];
...
/** All registered definitions (every key + version). */
export function getNotificationTypes(): NotificationTypeDefinition[] {
  return [...registry.values()];
}

/**
 * The highest-version definition per key — the ACTIVE routing config. Historical
 * versions stay registered as read-only renderers but no longer govern channels,
 * so retiring a channel doesn't fail startup just because an old version named it.
 */
export function getCurrentNotificationTypes(): NotificationTypeDefinition[] {
  const current = new Map<string, NotificationTypeDefinition>();
  for (const def of registry.values()) {
    const existing = current.get(def.key);
    if (!existing || def.version > existing.version) {
      current.set(def.key, def);
    }
  }
