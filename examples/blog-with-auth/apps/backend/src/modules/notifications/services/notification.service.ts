...
import { resolveActionUrl } from './notification-action-router.js';
...
function toRenderedContent(
  ...
  return {
    ...
    actionUrl: resolveActionUrl(content.action),
  };
}
...
/**
 * Deliver to each registered channel, isolated: a channel that throws is logged
 * and never fails `notify()` (the row is already persisted).
 */
async function dispatchToChannels(
  channelKeys: string[],
  resolved: Parameters<
    NonNullable<ReturnType<typeof getChannel>>['deliver']
  >[0],
): Promise<void> {
  const deliveries = channelKeys.map(async (channelKey) => {
    const channel = getChannel(channelKey);
    if (!channel) return;
    await channel.deliver(resolved);
  });
